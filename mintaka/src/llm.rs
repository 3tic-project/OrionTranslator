...
struct AliasInfo {
    name: String,
    count: usize,
    mentions: Vec<crate::detector::Mention>,
}
    ...
    pub async fn translate_all(
        ...
        for cluster in clusters {
            ...
            let handle = tokio::spawn(async move {
                ...
                translate_cluster(&client, &api_url, &api_key, &model, cluster).await
            });
            handles.push(handle);
        }
...
pub(crate) fn canonical_key(name: &str) -> String {
    strip_affixes(name)
}

pub(crate) fn strip_affixes(name: &str) -> String {
...
pub(crate) fn is_pure_title(name: &str) -> bool {
...
pub(crate) fn contains_kana(s: &str) -> bool {
    s.chars().any(|c| {
        let u = c as u32;
...
fn build_context_for_cluster(cluster: &NameCluster) -> serde_json::Value {
    ...
    let mut scored: Vec<(i32, &crate::detector::Mention)> = primary
        .mentions
        .iter()
...
fn context_item_text(m: &crate::detector::Mention) -> String {
...
fn mention_score(m: &crate::detector::Mention) -> i32 {
    let t = context_item_text(m);
    let mut score = 0i32;
...
fn build_alias_dst(cluster_key: &str, base_dst: &str, alias: &str) -> String {
    ...
    if let Some(rest) = alias.strip_prefix(cluster_key) {
        ...
        suffix = suffix.replace('\u{3000}', "").trim().to_string();
        if suffix.is_empty() {
            return base_dst.to_string();
        }
...
mod tests {
    ...
    fn test_contains_kana() {
        assert!(contains_kana("ネギ"));
        assert!(contains_kana("ねぎ"));
        assert!(!contains_kana("涅吉酱"));
        assert!(!contains_kana("志喜屋前辈"));
    }

    #[test]
    fn test_is_pure_title() {
        assert!(is_pure_title("先生"));
        assert!(is_pure_title("部長さん"));
        assert!(is_pure_title("お兄様"));
        assert!(!is_pure_title("志喜屋先輩"));
        assert!(!is_pure_title("温水君"));
    }

    #[test]
    fn test_strip_affixes() {
        assert_eq!(strip_affixes("佳樹さん"), "佳樹");
        assert_eq!(strip_affixes("志喜屋先輩"), "志喜屋");
        assert_eq!(strip_affixes("部長さん"), "部長");
        assert_eq!(strip_affixes("お兄様"), "兄");
    }
    ...
    fn m(line: usize, text: &str) -> crate::detector::Mention {
        crate::detector::Mention {
            line,
            line_text: text.to_string(),
            above: vec!["".to_string(); 5],
            follow: vec!["".to_string(); 5],
            confidence: 0.99,
            score: 0.99,
        }
    }
    ...
    #[test]
    fn test_build_name_clusters_merges_aliases() {
        let mut characters: HashMap<String, crate::detector::CharacterInfo> = HashMap::new();
        characters.insert(
            "佳樹".to_string(),
            crate::detector::CharacterInfo {
                name: "佳樹".to_string(),
                count: 3,
                content: vec![m(10, "佳樹が笑った")],
            },
        );
        characters.insert(
            "佳樹さん".to_string(),
            crate::detector::CharacterInfo {
                name: "佳樹さん".to_string(),
                count: 2,
                content: vec![m(20, "佳樹さん、と呼ばれた女性が手を上げる")],
            },
        );

        let clusters = build_name_clusters(&characters);
        let c = clusters.iter().find(|c| c.key == "佳樹").unwrap();
        assert_eq!(c.aliases.len(), 2);
    }

    #[test]
    fn test_build_context_prefers_identity_clues() {
        ...
        let ctx = build_context_for_cluster(&cluster);
        let arr = ctx.as_array().unwrap();
        let first = arr.first().unwrap();
        let text = first.get("text").unwrap().as_str().unwrap();
        assert!(text.contains("と呼ばれ"));
    }

    #[test]
    fn test_pick_primary_alias_prefers_gender_hint() {
        let aliases = vec![
            AliasInfo {
                name: "朝雲".to_string(),
                count: 10,
                mentions: vec![m(10, "朝雲が立った")],
            },
            AliasInfo {
                name: "朝雲君".to_string(),
                count: 3,
                mentions: vec![m(12, "朝雲君、と呼ばれた")],
            },
        ];

        let p = pick_primary_alias("朝雲", &aliases);
        assert_eq!(p, "朝雲君");
    }

    #[test]
    fn test_pick_primary_alias_prefers_gender_hint_over_high_count() {
        let aliases = vec![
            AliasInfo {
                name: "朝雲さん".to_string(),
                count: 50,
                mentions: vec![m(10, "朝雲さんが立った")],
            },
            AliasInfo {
                name: "朝雲君".to_string(),
                count: 2,
                mentions: vec![m(12, "朝雲君、と呼ばれた男性が手を上げる")],
            },
        ];
